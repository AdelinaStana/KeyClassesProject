...
public class DirectTopLinksCalculator implements Calculator {
	...
	public DirectTopLinksCalculator(DSM dsm, Set<Integer> top) {
		this.dsm = dsm;
		this.k = k;
		this.topclasses = top;
		setGraphMatrix();
	...
	public Map<Integer, Double> computeLinksNumber() {
	...
	public List<RankingEntry> getResultAsRankingEntryList() {
		...
		for (Map.Entry<Integer, Double> resultEntry : result.entrySet()) {
			...
			rankingEntry.setResultValue(ClassRankingProperties.DIRECTTOP, resultEntry.getValue());
	...
	public void addResultToRanking(List<RankingEntry> ranking) {
		if (ranking != null && ranking.size() > 0) {
			for (RankingEntry rankingEntry : ranking) {
				rankingEntry.setResultValue(ClassRankingProperties.DIRECTTOP, result.get(rankingEntry.getClassNumber()));
